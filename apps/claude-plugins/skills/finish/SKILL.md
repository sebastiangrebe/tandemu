...
RESULT=$(curl -sf -X POST "$TANDEMU_API/api/telemetry/tasks/<taskId>/finish" \
  ...
  -d '{
    ...
    "repo": "<owner/repo from git remote, e.g. 'sebastiangrebe/tandemu'>"
  }')
echo "$RESULT"
```
...
**IMPORTANT: The `add_memory` MCP tool uses a `text` parameter for the memory content. Always include metadata on every call.** This enables the dashboard to organize memories by repo, category, task, and source:
